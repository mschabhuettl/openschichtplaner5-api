...
class TestCacheInvalidation:
    ...
    def test_cache_invalidated_after_employee_create(self, sync_client):
        ...
        assert new_id in ids_after, (
            "Neuer Mitarbeiter nicht in Liste nach Cache-Invalidierung"
        )
...
class TestSSEBroadcastOnWrite:
    ...
    def test_broadcast_called_on_absence_create(self, sync_client, app):
        ...
        assert len(broadcast_calls) > 0, (
            "broadcast() wurde nach POST /api/absences nicht aufgerufen"
        )
    ...
    def test_broadcast_called_on_schedule_write(self, sync_client):
        ...
        assert resp.status_code in (200, 201, 204, 400, 404, 422), (
            f"Unexpected: {resp.status_code}"
        )
        if resp.status_code in (200, 201, 204):
            types = [c["type"] for c in broadcast_calls]
            assert "schedule_changed" in types, (
                "broadcast() nicht aufgerufen nach schedule write"
            )
    ...
    def test_me_employee_matches_by_name(self, app, sync_client):
        ...
        try:
            ...
            if data["employee"] is not None:
                assert (
                    data["employee"]["NAME"].strip().lower() == emp_name.strip().lower()
                )
        finally:
            _remove_token(tok)
...
class TestSelfServiceWishes:
    ...
    def test_delete_nonexistent_wish(self, app, sync_client):
        ...
        try:
            client = TestClient(app, raise_server_exceptions=False)
            resp = client.delete(
                "/api/self/wishes/999999", headers={"X-Auth-Token": tok}
            )
            assert resp.status_code == 404
        finally:
            _remove_token(tok)
