...
class TestPlanungsWorkflow:
    ...
    def test_full_planning_workflow(self, app, patched_db):
        ...
        resp = client.post(
            ...
            json={
                "entries": [
                    {"employee_id": emp_id, "date": test_date, "shift_id": shift_id}
                ],
                "overwrite": True,
            },
        )
        ...
        assert resp2.status_code in (200, 400, 500), (
            f"Unerwarteter Statuscode: {resp2.status_code}"
        )
...
class TestUrlaubsWorkflow:
    ...
    def test_full_vacation_workflow(self, app, patched_db):
        ...
        for lt in leave_types:
            ...
            short = (
                lt.get("KUERZEL") or lt.get("kuerzel") or lt.get("SHORT", "")
            ).lower()
        ...
        assert found, (
            f"Urlaubseintrag nicht in Abwesenheiten gefunden (emp={emp_id}, date={test_date})"
        )
...
class TestAutoplanerWorkflow:
    ...
    def test_full_autoplaner_workflow(self, app, patched_db):
        ...
        if cycles:
            ...
            assign_resp = planer_client.post(
            ...
            assert restr_resp.status_code in (200, 400), (
                f"Restriction fehlerhaft: {restr_resp.text}"
            )

        # 4. Schedule generieren (dry_run=True → kein Schreiben, nur Preview)
        gen_resp = planer_client.post(
        ...
        assert gen_resp.status_code == 200, (
            f"Schedule-Generierung fehlgeschlagen: {gen_resp.text}"
        )
    ...
    def test_wish_create_and_list(self, app, patched_db):
        ...
        assert wish_resp.status_code == 200, (
            f"Wunsch anlegen fehlgeschlagen: {wish_resp.text}"
        )
...
class TestBackupRestoreWorkflow:
    ...
    def test_backup_restore_roundtrip(self, app, patched_db):
        ...
        assert restore_resp.status_code == 200, (
            f"Restore fehlgeschlagen: {restore_resp.text}"
        )
    ...
    def test_backup_non_admin_forbidden(self, app, patched_db):
        ...
        assert resp.status_code == 403, (
            f"Leser sollte kein Backup erstellen dürfen: {resp.status_code}"
        )
...
class TestAuthWorkflow:
    ...
    def test_login_creates_valid_token(self, app, patched_db):
        ...
        assert create_resp.status_code == 200, (
            f"User anlegen fehlgeschlagen: {create_resp.text}"
        )
        ...
        try:
            ...
            assert login_resp.status_code == 200, (
                f"Login fehlgeschlagen: {login_resp.text}"
            )
            ...
            emp_resp = authed_client.get(
                "/api/employees", headers={"X-Auth-Token": token}
            )
            assert emp_resp.status_code == 200, (
                f"API-Aufruf mit Token fehlgeschlagen: {emp_resp.text}"
            )

            # 3. Logout
            logout_resp = authed_client.post(
            ...
            invalid_resp = authed_client.get(
                "/api/employees", headers={"X-Auth-Token": token}
            )
    ...
    def test_role_based_access_control(self, app, patched_db):
        ...
        resp_planer = planer_client.post(
            "/api/schedule/bulk",
            json={
                "entries": [
                    {
