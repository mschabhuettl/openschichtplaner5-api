...
class TestAuthLogin:
    def test_login_success_mocked(self, write_client):
        ...
        with (
            patch.object(main_module, "get_db") as mock_get_db,
    ...
    def test_logout_with_invalid_token(self, write_client):
        ...
        resp = write_client.post(
            "/api/auth/logout", headers={"x-auth-token": "nonexistent-token-xyz"}
        )
        assert resp.status_code == 200
...
class TestScheduleCRUDAPI:
    ...
    def test_add_schedule_entry_api(self, write_client):
        ...
        resp = write_client.post(
            "/api/schedule",
            json={"employee_id": emp_id, "date": "2025-10-01", "shift_id": shift_id},
        )
        assert resp.status_code == 200
        assert resp.json()["ok"] is True
    ...
    def test_add_schedule_entry_duplicate_conflict(self, write_client):
        ...
        write_client.post(
            "/api/schedule",
            json={"employee_id": emp_id, "date": "2025-10-05", "shift_id": shift_id},
        )
    ...
    def test_delete_schedule_entry_api(self, write_client):
        ...
        write_client.post(
            "/api/schedule",
            json={"employee_id": emp_id, "date": "2025-10-10", "shift_id": shift_id},
        )
...
class TestScheduleGenerateValidation:
    def test_generate_invalid_month(self, write_client):
        ...
        resp = write_client.post(
            "/api/schedule/generate", json={"year": 2025, "month": 13}
        )
        assert resp.status_code in (400, 422)
    ...
    def test_generate_force_param(self, write_client):
        ...
        resp = write_client.post(
            ...
            json={"year": 2025, "month": 8, "force": True, "dry_run": True},
        )
        assert resp.status_code == 200
...
class TestExportWith2026Data:
    ...
    def test_export_absences_with_group_2026(self, sync_client):
        ...
        resp = sync_client.get(
            f"/api/export/absences?year=2026&group_id={gid}&format=html"
        )
        assert resp.status_code == 200
...
class TestEmployeeCRUDFull:
    def test_create_employee_full(self, write_client):
        ...
        # Update
        put = write_client.put(
            f"/api/employees/{emp_id}", json={"FIRSTNAME": "Updated"}
        )
...
class TestAbsencesAPI:
    ...
    def test_absences_delete_by_emp_date(self, write_client):
        ...
        write_client.post(
            "/api/absences",
            json={"employee_id": emp_id, "date": "2025-11-01", "leave_type_id": lt_id},
        )
        # Delete it
