...
def _make_clients(app, tmp_path, notif_file_path):
    ...
    _sessions[admin_tok] = {"ID": 1, "NAME": "admin", "role": "Admin", "ADMIN": True, "RIGHTS": 255}
    _sessions[planer_tok] = {"ID": 2, "NAME": "planer", "role": "Planer", "ADMIN": False, "RIGHTS": 2}
    _sessions[emp_tok] = {"ID": 3, "NAME": "emp", "role": "Employee", "ADMIN": False, "RIGHTS": 1}
...
class TestCreateNotification:
    ...
    def test_creates_with_recipient(self, notif_env):
        ...
        entry = nm.create_notification(
            type="warn", title="Warn", message="msg", recipient_employee_id=42, link="/foo"
        )
...
class TestListNotifications:
    ...
    def test_filter_by_employee_id(self, notif_env):
        admin_c, _, _, notif_path = notif_env
        import api.routers.notifications as nm
        nm.create_notification(type="info", title="T", message="M", recipient_employee_id=5)
    ...
    def test_non_admin_cannot_see_other_employee_notifs(self, notif_env):
        ...
        import api.routers.notifications as nm
        nm.create_notification(type="info", title="T", message="M", recipient_employee_id=99)
    ...
    def test_non_admin_can_see_own_notifs(self, notif_env):
        _, planer_c, _, _ = notif_env
        import api.routers.notifications as nm
        nm.create_notification(type="info", title="T", message="M", recipient_employee_id=2)
...
class TestListAllNotifications:
    def test_admin_sees_all(self, notif_env):
        ...
        import api.routers.notifications as nm
        nm.create_notification(type="info", title="T", message="M")
        nm.create_notification(type="info", title="T2", message="M2", recipient_employee_id=5)
...
class TestMarkRead:
    ...
    def test_non_admin_cannot_mark_others(self, notif_env):
        ...
        e = nm.create_notification(type="info", title="T", message="M", recipient_employee_id=99)
    ...
    def test_non_admin_can_mark_own(self, notif_env):
        admin_c, planer_c, _, _ = notif_env
        import api.routers.notifications as nm
        e = nm.create_notification(type="info", title="T", message="M", recipient_employee_id=2)
        r = planer_c.patch(f"/api/notifications/{e['id']}/read")
        assert r.status_code == 200
